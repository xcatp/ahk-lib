...
FileObjLength := FileObject.Length
BRA := Buffer(FileObjLength)
...
WinHeight := Floor(Round(WinWidth / 4))
...
UpdateLayeredWindow(hwnd1, hdc, Floor((WAWidth - WinWidth) / 2), Floor((WAHeight - WinHeight) / 2), Floor(WinWidth), Floor(WinHeight))
...
UpdateTime() {
	...
	;AHK v1
	;FormatTime, Time
	Time :=  FormatTime(, "ddd MMMMdd日, yyyy hh:mm:ss tt")
	if (Time = OldTime)
		return
